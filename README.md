...
- 🎯 Entry points to execute your program using `python -m <pandaapi>` or `$ pandaapi` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/thetruerailen/PandaAPI/branch/main/graph/badge.svg?token=PandaAPI_token_here)](https://codecov.io/gh/thetruerailen/PandaAPI)
[![CI](https://github.com/thetruerailen/PandaAPI/actions/workflows/main.yml/badge.svg)](https://github.com/thetruerailen/PandaAPI/actions/workflows/main.yml)

Awesome pandaapi created by thetruerailen

## Install

from source
```bash
git clone https://github.com/thetruerailen/PandaAPI pandaapi
cd pandaapi
make install
```
...
```bash
pip install pandaapi
```

## Executing

```bash
$ pandaapi run --port 8080
```

or

```bash
python -m pandaapi run --port 8080
```

or

```bash
$ uvicorn pandaapi:app
```
...
```bash
❯ pandaapi --help
Usage: pandaapi [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ pandaapi create-user --help
Usage: pandaapi create-user [OPTIONS] USERNAME PASSWORD
...
```bash
pandaapi create-user admin admin --superuser
```
...
Run with `pandaapi run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `pandaapi create-user`.
...
-----------------------------------------------------
pandaapi/__init__.py              4      0   100%
pandaapi/app.py                  16      1    94%
pandaapi/cli.py                  21      0   100%
pandaapi/config.py                5      0   100%
pandaapi/db.py                   10      0   100%
pandaapi/models/__init__.py       0      0   100%
pandaapi/models/content.py       47      1    98%
pandaapi/routes/__init__.py      11      0   100%
pandaapi/routes/content.py       52     25    52%
pandaapi/routes/security.py      15      1    93%
pandaapi/routes/user.py          52     26    50%
pandaapi/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from pandaapi.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in pandaapi/default.toml.

### As environment variables
```bash
export pandaapi_KEY=value
export pandaapi_KEY="@int 42"
export pandaapi_KEY="@jinja {{ this.db.uri }}"
export pandaapi_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
pandaapi_ENV=production pandaapi run
```
